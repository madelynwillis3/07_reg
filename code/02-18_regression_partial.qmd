...
Because both the response variable (i.e., corn yield) and explanatory variable (i.e., seeding rate) are **numerical**, we can analyze this in a **regression** approach (instead of ANOVA).  
 (note: if x variables are categorical, you cannot do a regression. But if numerical, you can do either ANOVA (factor the x) or regression)
...
reg_dfw <- read_csv("../data/reg.csv") %>%
  clean_names() %>%
  mutate(rep = factor(rep)) #probably on exam

reg_dfw
```

This study was a randomized complete block design (RCBD) with four blocks.  
(note: factor level is different from treatment level)
...
#note: boxplot does not treat SR as factorial bc we didn't set as such. As a result it's just one big boxplot.
...
mod1_int <- lmer(yield_mgha ~ (1|rep) + 1,
                 data = reg_dfw) #rep is random part of RCBD
...
mod1_int_aug <- augment(mod1_int) %>%
  mutate(.stdresid = resid(mod1_int, 
                           type = "pearson", 
                           scaled = T)) %>%
  left_join(reg_dfw) #add back Seeding Rate since it wasn't included in the model.
...
```{r nd}
nd <- data.frame(sr_ksha = seq(40,120,1)) #from 40 to 120, sequence of 1.
...
# Model fitting
mod2_lin <- lmer(yield_mgha ~ (1|rep) + sr_ksha, #adding seeding rate to the model :)
                 data = reg_dfw)
